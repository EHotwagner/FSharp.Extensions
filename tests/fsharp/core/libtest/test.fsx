...
module FloatingPointStructured = begin
    ...
    let _ = check "d3wiojd31o" ((W Double.NaN < W 1.0)) true
    let _ = check "d3wiojd31p" ((W Double.NaN <= W 1.0)) true
    let _ = check "d3wiojd31a" ((W Double.NaN = W 1.0)) false
    let _ = check "d3wiojd31s" ((W Double.NaN <> W 1.0)) true
    let _ = check "d3wiojd31d" ((W 1.0 > W Double.NaN)) true
    let _ = check "d3wiojd31f" ((W 1.0 >= W Double.NaN)) true
...
module FloatingPointStructuredPoly = begin
    ...
    let _ = check "d3wiojd32o" ((W Double.NaN < W 1.0)) true
    let _ = check "d3wiojd32p" ((W Double.NaN <= W 1.0)) true
    let _ = check "d3wiojd32a" ((W Double.NaN = W 1.0)) false
    let _ = check "d3wiojd32s" ((W Double.NaN <> W 1.0)) true
    let _ = check "d3wiojd32d" ((W 1.0 > W Double.NaN)) true
    let _ = check "d3wiojd32f" ((W 1.0 >= W Double.NaN)) true
        ...
        for x in floats do
          for y in floats do
            ...
            if (xnan && not(ynan)) || (ynan && not(xnan)) then (
                ...
                let testRel x y = 
                    test1 x y "<" ((x < y) = not ynan );
                    test1 x y ">" ((x > y) = not xnan );
                    test1 x y ">=" ((x >= y) = not xnan);
                    test1 x y "<=" ((x <= y) = not ynan) in
                testEq x y; 
                testEq [x] [y];
                testEq [| x |] [| y |];
                testEq (x,x) (y,y);
...
do assert(xxs = checkIEnumerable xie)
do printf "Test c2eh2\n"; stdout.Flush(); let pred x = x<4                 in assert(List.choose (fun x -> if pred x then Some x else None)  xxs = checkIEnumerable (Seq.choose (fun x -> if pred x then Some x else None) xie))
do printf "Test c2e23ch2\n"; stdout.Flush();  let pred x = x<4                 in assert(List.filter pred xxs = checkIEnumerable (Seq.filter pred xie))
do printf "Test cc42eh2\n"; stdout.Flush();  let pred x = x%3=0               in assert(List.filter pred xxs = checkIEnumerable (Seq.filter pred xie))
do printf "Test c2f3eh2\n"; stdout.Flush();  let pred x = x%3=0               in assert(List.choose (fun x -> if pred x then Some x else None)  xxs = checkIEnumerable (Seq.choose (fun x -> if pred x then Some x else None) xie))
do printf "Test c2eh2\n"; stdout.Flush();  let pred x = x>100               in assert(List.filter pred xxs = checkIEnumerable (Seq.filter pred xie))
do printf "Test c2egr3h2\n"; stdout.Flush();  let pred x = x>100               in assert(List.choose (fun x -> if pred x then Some x else None)  xxs = checkIEnumerable (Seq.choose (fun x -> if pred x then Some x else None) xie))
do printf "Test c2eh2\n"; stdout.Flush();  let f   x  = x*2                 in assert(List.map  f xxs = checkIEnumerable (Seq.map  f xie))
do printf "Test cvaw2eh2\n"; stdout.Flush();  let f   x  = x*2                 in assert([ 2;3 ] = 
                                              checkIEnumerable (Seq.generate (fun () -> ref 1) (fun r -> incr r; if !r > 3 then None else Some(!r)) (fun r -> ())))
do printf "Test c2r5eh2\n"; stdout.Flush();  let f i x  = x*20+i              in assert(List.mapi f xxs = checkIEnumerable (Seq.mapi f xie))
do printf "Test c2vreeh2\n"; stdout.Flush();  let f _ x  = x*x                 in assert(List.map2 f xxs xxs = checkIEnumerable (Seq.map2 f xie xie))
do printf "Test c2vreeh2\n"; stdout.Flush();  let f _ x  = x*x                 in assert(List.map2 f xxs xxs = checkIEnumerable (Seq.map2 f xie xie))
do let f _ x  = x*x                 in assert(List.concat [] = checkIEnumerable (Seq.concat [| |]))
do let f _ x  = x*x                 in assert(List.concat [xxs] = checkIEnumerable (Seq.concat [|  xie |]))
do let f _ x  = x*x                 in assert(List.concat [xxs;xxs] = checkIEnumerable (Seq.concat [| xie; xie |]))
do let f _ x  = x*x                 in assert(List.concat [xxs;xxs;xxs] = checkIEnumerable (Seq.concat [| xie; xie;xie |]))
do printf "Test c25reh2\n"; stdout.Flush();  let f _ x  = x*x                 in assert(List.append xxs xxs = checkIEnumerable (Seq.append xie xie))
do printf "Test c27mog7keh2\n"; stdout.Flush();  let f x    = if x%2 =0 then Some x
                                                              else None in assert(List.choose f xxs = checkIEnumerable (Seq.choose f xie))
do printf "Test c2e8,h2\n"; stdout.Flush();  let f z x  = (z+1) * x % 1397    in assert(List.fold f 2 xxs = Seq.fold f 2 xie)
...
do printf "Test c2grgeh2\n"; stdout.Flush();  assert(List.nth xxs 3 = Seq.nth 3 xie)  (* err: args wrong way around for List.nth *)
...
do check "clwnwe95" (10.000m.ToString()) "10.000"
