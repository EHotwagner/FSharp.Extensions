...
module NameClashesWithDefaultAugmentation = begin
    type T = | C of int * int
             | D of (int * int)
...
module CHeckErrorForNamedArgumentToDatatype = 
    ...
    let x = 1
    let c1 = C1 (x=1, x = 1) // error expected
    let c2 = C2 (x=1) // no error expected. Forward compat restriction only applies to multiple arguments
